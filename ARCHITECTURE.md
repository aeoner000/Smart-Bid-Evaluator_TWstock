...
2. 模型訓練管道 (run_training_pipeline)
   └─ 對每個目標變量 (per-target):
      ...
      ├─ 訓練多個模型，比較測試集 RMSE
      ├─ 選擇性能最佳的模型為 champion
      ├─ 儲存模型為 {y_english}_best_model.joblib-->含模型特徵及基礎資訊
...
| **Process** | 原始表 + 配置 | all_features (寬表檢查點) Train_*/Test_*/Predict_* | 數據庫 + .joblib 物件 |
| **Decision** | 數據量查詢 | 訓練決策 (True/False) | — |
| **Train** | Train_*/Test_* + 物件 | 已訓練模型 + 元數據 | .joblib 物件 + training_metadata.json |
| **Predict** | Predict_table + 物件 | 預測結果 | 數據庫預測表 |
...
| **訓練觸發閾值** | 新數據筆數 ≥ 30 | 積累足夠新樣本，衡量計算成本與模型新鮮度的平衡 |
...
若訓練過程失敗（無法達到收斂等）或改善無達5%以上，系統保留現有最佳模型，避免回歸。
